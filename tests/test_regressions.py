...
class TestRegressions(unittest.TestCase):
    ...
    def test_legacy_v1_6_0_with_aws_v1_0_0(self):
        # The legacy schema export, even with v3 fixes, changes a slightly different schema, however these differences
        # are not material differences in actual usage. The test uses a diff callback to ensure these differences
        # are ones we expect.

        # Compile using legacy mode with scoped keys to minimize the differences.
        compiler = SchemaCompiler(Path(BASE_DIR, 'uncompiled-schemas/ocsf-schema-v1.6.0'),
                                  extensions_paths=[Path(BASE_DIR, 'uncompiled-schemas/aws-v1.0.0')],
                                  legacy_mode=True, scope_extension_keys=True)
        schema = compiler.compile()
        baseline_schema = read_json_object_file(Path(BASE_DIR,
                                                     'compiled-baselines/server-v3-schema-v1.6.0-aws-v1.0.0.json'))
        ok, diffs = diff_objects(schema, baseline_schema, diff_callback=legacy_aws_diff_callback)
        self.assertTrue(ok, f"schema should match baseline:\n{formatted_diffs(diffs)}")
...
def legacy_aws_diff_callback(
    ...
    # This compiler overwrites last_used_time, and then the scope_extension_keys option it to aws/last_user_time
    if (key == "dictionary_attributes"
        and isinstance(left_diff, DiffDictKeys) and left_diff.keys == []
        and isinstance(right_diff, DiffDictKeys) and right_diff.keys == ["last_used_time", "last_used_time_dt"]):
        return True

    if ((path == ["dictionary_attributes", "last_used_time"] or path == ["dictionary_attributes", "last_used_time_dt"])
        and left_diff == MISSING):
        # These will be missing with this compiler and scope_extension_keys (left value)
        return True

    if (len(path) == 2 and path[0] == "objects"
        and isinstance(left_diff, DiffDictKeys) and left_diff.keys == []
        and isinstance(right_diff, DiffDictKeys) and right_diff.keys == ["profiles"]):
        # This compiler does not needlessly set object and class "profiles" to null.
        return True

    if key == "profiles" and len(path) == 3 and path[0] == "objects" and left_diff == MISSING and right_diff is None:
        # Same as above but now we are at the "objects.<object-name>.profiles" level
        return True

    if (key in ["last_used_time", "last_used_time_dt"] and len(path) > 2 and path[-2] == "attributes"
        and isinstance(left_diff, DiffDictKeys) and left_diff.keys == ["extension", "extension_id"]
        and isinstance(right_diff, DiffDictKeys) and right_diff.keys == []):
        # This compiler overwrites the one last_user_time (which is carried over to last_user_time_dt)
        # and so class and object attributes using it will have "extension" and "extension_id"
        return True

    if (key in ["extension", "extension_id"] and len(path) > 3 and path[-3] == "attributes"
    ...
    if (key == "caption" and len(path) > 3 and path[-3] == "attributes"
        and path[-2] in ["last_used_time", "last_used_time_dt"]
        and left_diff == "Last Used time" and right_diff == "Last Used Time"):
        # Because this compiler overwrite the actual dictionary attribute last_used_time, the caption in object
        # attributes (and class attributes if that happened) becomes the one from the AWS extension.
        return True
